...
// use crate::pervasive::option::Option::Some;
// use crate::pervasive::option::Option::None;
...
impl<V> DList<V> {
    ...
    spec fn wf_perm(&self, i: nat) -> bool {
        ...
        && match self.perms.index(i).view().value {
            Some(node) => equal(node.prev, self.prev_of(i))
    ...
    spec fn well_formed(&self) -> bool {
        ...
        && if self.ptrs.len() == 0 {
            equal(self.head, None) && equal(self.tail, None)
        } else {
            equal(self.head, Some(self.ptrs.index(0))) &&
